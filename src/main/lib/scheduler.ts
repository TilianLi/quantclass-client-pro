...
const setupScheduler = async (): Promise<schedule.Job> => {
	...
	systemState.job = schedule.scheduleJob("* * * * *", async () => {
		...
		if (systemState.isSetAutoUpdate) {
			...
			// -- T-20260906-10：fusion 回测进行中时本轮数据更新让行（写/读 CSV 竞态会致
			//    fusion 静默死亡）；下轮 15 分钟自动再试，不改变盘中 fuel/fusion 并行的原设计
			// -- 4.2.1 起 zeus 并入 fusion 内核，原 isKernalBusy("zeus") 改查 fusion
			const isZeusBusy = await isKernalBusy("fusion")
			if (isFuelBusy && !allowConcurrentFuelTasks) {
				logger.info("[fuel] 内核正忙，跳过本轮调度")
			} else if (isZeusBusy) {
				logger.info(
					"[fuel] fusion 回测进行中，跳过本轮数据更新（T-20260906-10 防竞态）",
				)
